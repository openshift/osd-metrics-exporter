...
git_hash = sys.argv[4]
osd_metrics_exporter_image = sys.argv[5]

full_version = "%s.%s-%s" % (VERSION_BASE, git_num_commits, git_hash)
print("Generating CSV for version: %s" % full_version)

with open('config/templates/osd-metrics-exporter-csv-template.yaml', 'r') as stream:
...
csv['spec']['install']['spec']['deployments'][0]['spec']['template']['spec']['containers'][0]['image'] = osd_metrics_exporter_image

# Update the versions to include git hash:
csv['metadata']['name'] = "osd-metrics-exporter.v%s" % full_version
csv['spec']['version'] = full_version
csv['spec']['replaces'] = "osd-metrics-exporter.v%s" % prev_version
...
csv_filename = "osd-metrics-exporter.v%s.clusterserviceversion.yaml" % full_version
